...
- renders grouped section headers as containing cards with progress-aware video cards beneath them, keeps the expand/collapse control in the top left of the group card, lets the whole header toggle expansion, uses a solid readable collapsed header with only the staggered bottom edges of up to three same-width full-size stacked cards peeking from a clipped reveal beneath the front card in equal reveal steps, uses a thumbnail-forward content-derived full-card mosaic only on the expanded group header itself, keeps the child episode cards on their normal single-thumbnail treatment, keeps the expanded title and group info inside a readable wider bottom info plate so the upper card body still reads as artwork, places the single group watched-state toggle immediately above the child video list, uses a custom progress bar that avoids ambiguous endpoint dots on video cards, and provides a right-side jump rail that anchors to the episode-list panel for quick section navigation based on the currently visible section anchors without overlapping card text or interactive controls or painting a full-screen overlay behind the rail
