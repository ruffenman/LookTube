...
- grouped headers can collapse or expand independently with compact icon-based expand/collapse controls, allow the entire header to toggle expansion, keep dimmed art visible while collapsed with same-width stacked-card peeks, use a subdued neutral collapsed style and a content-derived expanded full-card mosaic with a readable bottom info plate, place their title and metadata beneath that control instead of offsetting them to its right, expose one whole-group watched-state toggle at a time, keep overview controls for expanding or collapsing all groups, render as containing cards, and visually nest their episode cards beneath the owning header
