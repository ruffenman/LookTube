...
- grouped headers can collapse or expand independently with compact icon-based expand/collapse controls, allow the entire header to toggle expansion, keep dimmed art visible while collapsed with the lead thumbnail on the front card and same-width stacked-card peeks from later videos, use a subdued neutral collapsed style and a content-derived expanded full-card frame-tile mosaic with a tighter readable bottom info plate that stays visually separated from the affordance, expose one whole-group watched-state toggle at a time, keep overview controls for expanding or collapsing all groups, render as containing cards, and visually nest their episode cards beneath the owning header
